...
// products section
    ...
    app.get("/mechanic", async (req, res) => {
      try {
        const result = await mechanicsCollection.find({}).toArray();
        res.send(result);
      } catch (error) {
        console.log(error)
      }
    ...
    app.get("/mechanic/:id", async (req, res) => {
      const productId = req.params.id;
      const query = { _id: ObjectId(productId) };
      const result = await mechanicsCollection.findOne(query);
      res.json(result);
    });
    app.post("/mechanic", async (req, res) => {
      const product = req.body;
      const result = await mechanicsCollection.insertOne(product);
      res.json(result);
    });
